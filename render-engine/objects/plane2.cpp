...
GLuint plane2_mloc, plane2_vloc, plane2_ploc;
...
void initPlane2Buffers()
{
	...
	plane2_program = InitShader("objects/cube/cube_vshader.glsl", "objects/cube/cube_fshader.glsl");
	...
	plane2_mloc = glGetUniformLocation(plane2_program, "model");
	plane2_vloc = glGetUniformLocation(plane2_program, "view");
	plane2_ploc = glGetUniformLocation(plane2_program, "projection");

	glEnable(GL_DEPTH_TEST);
	glUseProgram(plane2_program);
}
...
{
	bind(plane2_program, plane2_VAO);
	useCameraMatrix(plane2_vloc, plane2_ploc);

	mat4 instance = TRS(position, rotation, scale);
	glUniformMatrix4fv(plane2_mloc, 1, GL_TRUE, plane2ModelMatrix * instance);

	setUniformVec4(plane2_program, "mainColor", color);
	useLights(plane2_program, "lightPosition", "lightColor", "viewPosition", CAM_POS_3);
